...
class MakeReportResult():
    ...
    def __init__(self, df_trades, df_dados, nome_arquivo = 'backtest.pdf', 
                 ...
                 caminho_benchmarks = r'C:\Users\rafae\OneDrive\Documentos\Bolsa de Valores\Modelos_Quantitativos\base_dados_br'):
        
        self.df_trades = df_trades
        
        self.df_trades['data'] =  self.df_trades['data'].dt.date
        
        self.df_trades['retorno'] = self.df_trades['retorno'] + 1
        self.df_trades = (
            self.df_trades.groupby('data')
                .agg({
                    'retorno': 'prod',
                    'numero_trade': 'last',
                    'sinal': 'last',
        ...
        self.df_trades =  self.df_trades.reset_index()
        self.df_trades['retorno'] = self.df_trades['retorno'] - 1
        ...
        self.dados['retorno'] = self.dados['retorno'] + 1
        self.dados = (
            self.dados.groupby('data')
                .agg({
                    'abertura': 'first',
                    'minima': 'min',
                    'maxima': 'max',
        ...
        if caminho_benchmarks == None:
            ...
            self.ibov = pd.read_parquet(r'C:\Users\rafae\OneDrive\Documentos\Bolsa de Valores\Modelos_Quantitativos\base_dados_br\ibov.parquet')
            self.cdi = pd.read_parquet(r'C:\Users\rafae\OneDrive\Documentos\Bolsa de Valores\Modelos_Quantitativos\base_dados_br\cdi.parquet')
    ...
    def estatisticas_de_trade(self):
        ...
        self.trades_acum = (self.df_trades.groupby(['numero_trade']).tail(1))['retorno_por_trade']   

        self.trades = self.trades_acum.dropna().unique()

        self.operacoes_vencedoras = np.sum(np.where(self.trades > 0, True, False))/self.numero_trades

        self.operacoes_perdedoras = 1 - self.operacoes_vencedoras

        self.media_ganhos =  self.trades[self.trades > 0].mean()

        self.media_perdas =  self.trades[self.trades < 0].mean()
        ...
        # print(list(g) if k else [] for k, g in groupby(self.trades.tolist(), key=lambda i: i > 0))
        
        if (list(g) if k else [] for k, g in groupby(self.trades.tolist(), key=lambda i: i > 0)) is not None:
            self.maior_sequencia_vitorias = len(max((list(g) if k else [] for k, g in groupby(self.trades.tolist(), key=lambda i: i > 0)), key=len))
        else:
            self.maior_sequencia_vitorias = 0
            
        if (list(g) if k else [] for k, g in groupby(self.trades.tolist(), key=lambda i: i < 0)) is not None:
            self.maior_sequencia_derrotas = len(max((list(g) if k else [] for k, g in groupby(self.trades.tolist(), key=lambda i: i < 0)), key=len))
        else:
            self.maior_sequencia_derrotas = 0
    ...
    def grafico_retorno_mes_a_mes(self):
        ...
        rent_mes = rent_mes[['Jan', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']]
